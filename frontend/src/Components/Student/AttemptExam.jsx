...
function AttemptExam() {
  ...
  const logIncident = async (type) => {
    try {
      await fetch('${apiUrl}/api/incidents/report', {
        method: 'POST',
  ...
  useEffect(() => {
    fetch(`${apiUrl}/api/exams/${examId}?attemptId=${attemptId}`, {
      credentials: 'include',
    })
      .then(async (res) => {
  ...
  useEffect(() => {
    const fetchAttempt = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/attempts/${attemptId}`, {
          credentials: 'include',
        });
        const data = await res.json();
...
setSelectedAnswers(initialAnswers);
  ...
  const handleAnswerChange = async (qid, answer) => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/attempts/${attemptId}/answer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch(`${apiUrl}/api/attempts/${attemptId}/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
